    ...
    if (!validateCsrfToken($csrf_token)) {
        ...
        switch ($_POST['admin_action'] ?? '') {
            ...
            case 'delete_user':
                ...
                $stmt = $conn->prepare("DELETE FROM reviews WHERE rater_user_id = ? OR rated_user_id = ?");
            ...
            case 'disable_review':
                ...
                $stmt = $conn->prepare("UPDATE reviews SET is_disabled = 1 WHERE id = ?");
            ...
            case 'enable_review':
                $review_id = (int)($_POST['review_id'] ?? 0);
                $stmt = $conn->prepare("UPDATE reviews SET is_disabled = 0 WHERE id = ?");
                $stmt->bind_param("i", $review_id);
                $stmt->execute();
...
if ($section === 'overview') {
    ...
    $stmt = $conn->query("SELECT COUNT(*) as total FROM users WHERE user_type = 'pet-sitter'");
    $stats['total_sitters'] = $stmt->fetch_assoc()['total'];
    
    $stmt = $conn->query("SELECT COUNT(*) as total FROM users WHERE user_type = 'pet-owner'");
    $stats['total_owners'] = $stmt->fetch_assoc()['total'];
    
    $stmt = $conn->query("SELECT COUNT(*) as total FROM reviews");
    $stats['total_reviews'] = $stmt->fetch_assoc()['total'];
}
